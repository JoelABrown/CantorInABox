...
// ********************************************************************
// JB: Added Volume and Pan to the settings that can be adjusted
//     Changed tempo range from -50% to +100% to 50% to 200%
// ********************************************************************
...
namespace SoundTouch.Net.NAudioSupport
{
    ...
    /// contents of a <see cref="WaveStream"/>.
    /// </summary>
    public class SoundTouchWaveStream : WaveStream
    {
        ...
        /// <summary>
        /// Gets or sets the Pan value (-1.0f full left, +1.0f full right)
        /// </summary>
        public float Pan
        {
            get
            {
                ...
                {
                    result = ((WaveChannel32)(_sourceStream)).Pan;
                }
                return result;
            }
            set
            {
                // Pan must be between -1.0f and +1.0f
                float newPanValue = Math.Min(Math.Max(-1.0f, value), 1.0f);
                if (_sourceStream is not null)
                {
                    ((WaveChannel32)(_sourceStream)).Pan = newPanValue;
                }
            }
        }

        /// <summary>
        /// Gets or sets the Volume value (0.0f to 1.0f)
        /// </summary>
        public float Volume
        {
            get
            {
                ...
                if (_sourceStream is not null)
                {
                    result = ((WaveChannel32)(_sourceStream)).Volume;
                }
                return result;
            }
            set
            {
                // Volume must be between 0.0f and +1.0f
                float newVolumeValue = Math.Min(Math.Max(0.0f, value), 1.0f);
                if (_sourceStream is not null)
                {
                    ((WaveChannel32)(_sourceStream)).Volume = newVolumeValue;
                }
            }
        }
        ...
        {
            get => _provider.TempoChange + 100.0;
            set => _provider.TempoChange = value - 100.0;
        }
